...
3. **其他交互场景** - 格式见对应引用文件（方案构思选择、测试失败、代码质量询问等）
...
**版本号确定优先级:**
1. 用户明确指定
2. 从主模块解析（按 `references/templates.md` 的 A3 查找表）
...
**说明**: 本规则定义知识库操作的调度逻辑。详细执行步骤见 `references/kb.md`。
...
STEP 2: 知识库不存在
  ...
  方案设计/开发实施阶段: 阅读 `references/kb.md` 执行完整创建流程

STEP 3: 知识库存在
  质量检查: 重度问题 → 阅读 `references/kb.md` 重建; 轻度问题 → 继续流程
```

#### 项目上下文获取策略

**快速流程**: 先检查知识库 → 不存在或信息不足则扫描代码库
**详细规则**: 见 `references/kb.md`
...
- 动作流程:
  ...
  4. 执行代码改动
  5. 同步更新知识库（按 `references/kb.md` 的同步规则）
  6. 迁移方案包至history/
  7. 扫描遗留方案
- 简化方案包规则:
  - 路径: `plan/YYYYMMDDHHMM_<feature>/`
...
**详细规则:** → 进入阶段时阅读 `references/analyze.md`
...
**详细规则:** → 进入阶段时阅读 `references/design.md`
...
**详细规则:** → 进入阶段时阅读 `references/develop.md`
...
| 路径/阶段 | 引用文件 | 触发时机 |
|----------|-----------|---------|
| 完整研发 / 需求分析 | `references/analyze.md` | 进入需求分析时阅读 |
| 标准开发/完整研发 / 方案设计 | `references/design.md` | 进入方案设计时阅读 |
| 所有开发模式 / 开发实施 | `references/develop.md` | 进入开发实施时阅读 |
| 知识库命令 / 知识库操作 | `references/kb.md` | ~init命令或知识库缺失时阅读 |
| 创建文件 | `references/templates.md` | 创建方案包/Wiki文件时阅读 |

**引用文件路径:** `references/`（相对于本 skill 目录）
