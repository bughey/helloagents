...
```yaml
定义:
  SKILL_ROOT: 本 Skill 根目录（SKILL.md 同级；也是 references/ 的父目录）

文件引用规则:
  - 统一使用从 SKILL_ROOT 起算的相对路径（符合 Agent Skills 规范）
  - 示例: references/stages/evaluate.md
  - 示例: scripts/upgradewiki.py
  - 示例: assets/templates/INDEX.md

一致性原则（CRITICAL）:
  - 所有模块必须来自同一个 SKILL_ROOT（避免版本不一致）
  - 禁止混用不同目录的模块文件（避免版本不一致）
  - 模块文件不存在时报错并停下，禁止静默跳过
```
...
执行流程:
  1. 确定 SKILL_ROOT（=本 Skill 根目录，仅首次执行）
