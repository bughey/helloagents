...
session.py:
  用法: python -X utf8 '{SCRIPTS_DIR}/rlm/session.py' --info|--list|--cleanup [<hours>]
  说明: RLM Session 管理（位于 rlm/ 目录，非 scripts/）

shared_tasks.py:
  用法: python -X utf8 '{SCRIPTS_DIR}/rlm/shared_tasks.py' --status|--list|--available|--claim <id> --owner <sid>|--complete <id>|--add '<subject>' [--blocked-by <ids>]
